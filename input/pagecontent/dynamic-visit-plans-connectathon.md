...
The representation of this as a JSON can be found [here]() 
...
graph TD
    ...
    %% Primary Path
    Randomization --OnStudy--> C1D1
    C1D28 --TreatmentContinuation--> C2D1
    C2D28 --TreatmentContinuation--> C1D1
    C2D28 --Progressive<br/>Disease--> EndTreatment
    C1D28 --Progressive<br/>Disease--> EndTreatment
    EndTreatment --> FU1
    FU2 --> EndOfStudy
    
    %% Early termination paths
    Screening --Screen<br/>Failure--> EndOfStudy
    Randomization --Patient<br/>Withdrawal--> EndOfStudy
    Cycle1 --Adverse<br/>Event--> PostTreatment
    Cycle2 --Adverse<br/>Event--> PostTreatment
    FollowUp --Lost to<br/>Follow-up--> EndStudy
    ...
    classDef encounter fill:#aaffaa,stroke:#666666,stroke-width:1px
